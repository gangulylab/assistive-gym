...
class DressingEnv(AssistiveEnv):
	def __init__(self, robot_type='jaco', human_control=False,):
		...
		self.mean_mass = (self.config('mass','human_male'),self.config('mass','human_female'))
		self.observation_space = spaces.Box(-np.inf,np.inf,(24,), dtype=np.float32)
		self.num_targets = 1

	def step(self, action):
		print(action)
		self.take_step(action, robot_arm='left', gains=self.config('robot_gains'), forces=self.config('robot_forces'), human_gains=0.0025)

		# Update robot position
		forces_torques = []
		for _ in range(self.frame_skip):
			# Force the cloth attachment to stay at the end effector
			state = p.getLinkState(self.robot, 8, computeForwardKinematics=True, physicsClientId=self.id)
		...
		# Essentially distance from shoulder
		...
		obs,human_obs,recreate_full_obs = self._get_obs(ft, [cloth_force_sum, robot_force_on_human])

		self.task_success = reward_dressing > self.config('task_success_threshold')

		total_force_on_human = robot_force_on_human + cloth_force_sum
		info = {
			# 'total_force_on_human': total_force_on_human, 
			'task_success': self.task_success,
			'dressing_quality': reward_dressing,
			'preferences_score': preferences_score,
			'human_obs': human_obs,
			'recreate_full_obs': recreate_full_obs,
			'joint_pos': obs[-8:-1],
			}
		done = False
		print(obs)
	...
	def _get_obs(self, forces, forces_human):
		...
		recreate_full_obs = lambda obs: np.concatenate([obs[:-1], shoulder_pos-torso_pos, elbow_pos-torso_pos, wrist_pos-torso_pos, forces]).ravel()
		if self.human_control:
		...
		return robot_obs, human_obs, recreate_full_obs

	def random_human(self):
		self.configp['human_male']['mass'] = np.clip(self.np_random.normal(0,10),-20,20) + self.mean_mass[0]
		self.configp['human_male']['radius_scale'] = self.np_random.uniform(.75,1)
		self.configp['human_male']['height_scale'] = self.np_random.uniform(.75,1)

		self.configp['human_female']['mass'] = np.clip(self.np_random.normal(0,10),-20,20) + self.mean_mass[1]
		self.configp['human_female']['radius_scale'] = self.np_random.uniform(.75,1)
		self.configp['human_female']['height_scale'] = self.np_random.uniform(.75,1)
	...
	@property
	def tool_pos(self):
		return np.array(p.getBasePositionAndOrientation(self.cloth_attachment, physicsClientId=self.id)[0])
